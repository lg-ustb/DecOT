...
def read_data():
    Y = pd.read_csv('../data/Y.csv', header=0, index_col=0)
    D = pd.read_csv('../data/SC.csv', header=0, index_col=0)
    weight_gene = pd.read_csv('../data/Weight.gene.csv', header=0, index_col=0)
    S = pd.read_csv('../data/S.csv', header=0, index_col=0)
    sigma = pd.read_csv('../data/sigma.csv', header=0, index_col=0)
    return np.array(Y), np.array(D), np.array(weight_gene), np.array(S), np.array(sigma)
...
def ground_cost(D):
    ...
    for i in range(n):
        for j in range(i):
            cost[i, j] = np.linalg.norm(D[i, :] - D[j, :])
            cost[j, i] = cost[i, j]
    return cost
...
def main():
    ...
    np.save('../temp/Y_selected.npy', Y)
    np.save('../temp/D_selected.npy', D)
    ...
    M = ot.dist(D, D, metric='euclidean')
    gamma = 1 / 100
    rho = 0.1
    ...
    lambda0, H, obj, gradient = wasserstein_NMF_coefficient_step(Y, K, D, gamma, rho, H, options)
    np.savetxt('../results/wasserstein_lambda0.txt', lambda0)
    np.save('../temp/lambda0.npy', lambda0)
    np.save('../temp/Y_normalized.npy', Y)
    np.save('../temp/D_normalized.npy', D)
    return lambda0, H, obj, gradient
